...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        if (sample == False):
            ...
            return math.sqrt(summation / len(self.data))
        summation = 0
        for value in self.data:
            summation += (value - mean) ** 2
        return math.sqrt(summation / (len(self.data) - 1))
    ...
    def pdf(self, x):
        ...
        numerator = math.exp(-0.5 * (((x - self.mean) / self.stdev) ** 2.0))
        denominator = self.stdev * math.sqrt(2.0 * math.pi)
